class EJS_GameManager {
	constructor(Module, EJS) {
		...
		// 连发相关
		this.autoFireTimers = [...Array(4).keys()].map(() => ({
			// 初始化4个玩家的连发定时器
			a_interval: null,
			a_timeout: null,
			b_interval: null,
			...
			if (this.EJS.config.externalFiles && this.EJS.config.externalFiles.constructor.name === 'Object') {
				for (const key in this.EJS.config.externalFiles) {
					await new Promise(done => {
						this.EJS.downloadFile(this.EJS.config.externalFiles[key], null, true, { responseType: 'arraybuffer', method: 'GET' }).then(
							async res => {
								if (res === -1) {
									if (this.EJS.debug)
										console.warn("Failed to fetch file from '" + this.EJS.config.externalFiles[key] + "'. Make sure the file exists.")
									return done()
								}
								let path = key
	...
	simulateInput(player, index, value) {
		...
		// 连发相关
		if ([1, 9].includes(index)) {
			// X键(index=9)触发A键(index=8)连发
			if (index === 9) {
				const playerTimers = this.autoFireTimers[player]
				let fire = () => {
					this.functions.simulateInput(player, 8, 1) // 按下A键
					playerTimers.a_timeout = setTimeout(() => {
						this.functions.simulateInput(player, 8, 0) // 释放A键
					}, this.EJS.autoFireDelay / 2)
				}
				let stop_autoFire = () => {
					this.functions.simulateInput(player, 8, 0) // 释放A键
					clearInterval(playerTimers.a_interval)
					clearTimeout(playerTimers.a_timeout)
				}
				if (value === 1) {
					//x键按下
					stop_autoFire()
					fire()
					playerTimers.a_interval = setInterval(() => {
						fire()
					}, this.EJS.autoFireDelay)
				} else {
					//x键松开
					stop_autoFire()
				}
			}
			// Y键(index=1)触发B键(index=0)连发
			if (index === 1) {
				const playerTimers = this.autoFireTimers[player]
				let fire = () => {
					this.functions.simulateInput(player, 0, 1) // 按下A键
					playerTimers.b_timeout = setTimeout(() => {
						this.functions.simulateInput(player, 0, 0) // 释放A键
					}, this.EJS.autoFireDelay / 2)
				}
				let stop_autoFire = () => {
					this.functions.simulateInput(player, 0, 0) // 释放A键
					clearInterval(playerTimers.b_interval)
					clearTimeout(playerTimers.b_timeout)
				}
				if (value === 1) {
					//y键按下
					stop_autoFire()
					fire()
					playerTimers.b_interval = setInterval(() => {
						fire()
					}, this.EJS.autoFireDelay)
				} else {
					//y键松开
