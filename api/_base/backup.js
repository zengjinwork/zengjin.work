...
export default async (req, resp) => {
  ...
  // 1. 鉴权：生产环境下强校验 Vercel Cron Secret
  const isProd = process.env.NODE_ENV === 'production';
  const authHeader = req.headers.authorization;
  if (isProd && authHeader !== `Bearer ${process.env.CRON_SECRET}`) {
    console.error('备份网关拦截：未授权的 Cron 请求');
    return base.respFailure({ msg: '未授权的请求' });
  }
  ...
  try {
    ...
    for (const table of targetTables) {
      ...
      // D. 重建唯一键
      const uniGroup = {};
      meta.constraints.filter(c => c.constraint_type === 'UNIQUE').forEach(c => {
